...
| trap | what happens |
| --- | --- |
| ⛔ **Chrome on Linux does not read the user's NSS database for server authentication.** | Adding a harness authority with `certutil -t "C,,"` and confirming it with `certutil -L` still produces `CertificateUnknown`, because the browser uses its own root store there. The way through is `--ignore-certificate-errors --test-type` passed to the **browser**, which changes what it accepts after the handshake rather than what it sends. ⚠ Never ship such a flag in a client. |
