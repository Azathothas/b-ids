...
⭐ **In this project it is the ordinary answer rather than an exotic one.** The
capture surface is bounded by which browsers a machine can be made to run, and
installing browsers on somebody's machine is a system change nobody asked for.
A throwaway machine answers in well under the time a CI round trip takes, and it
answers for a browser, a channel, a libc or a distribution this host does not
have.

A newer browser than the one installed. A libc this host does not use. A
different distribution's NSS build, which is a real source of variance for
Firefox. A filesystem it does not have. A kernel feature that has to be
registered and then unregistered. Each needs a different machine.
...
## ⛔ What a guest must never carry back

⚠ **A throwaway machine is throwaway in both directions.** The reason this
project drives a browser inside one is that the browser must have visited
nothing, hold no profile, and belong to nobody.

- ⛔ **A guest never receives a real browser profile, a cookie jar, a
  certificate store from a person's machine, or any credential.** Every browser
  it launches is launched into a directory the run created and the run destroys.
- ⛔ **Nothing captured inside a guest is published without passing the same
  rules as anything else in this tree.**
  [`security/secrets.md`](security/secrets.md) applies to a capture exactly as
  it applies to a document, and its public-repository section is the half that
  bites here.
- ⚠ **A guest's own network is not a private network.** Anything the guest
  fetches is a request somebody can see, and anything the guest is pointed at
  should be this host's listener rather than a third party's site.
