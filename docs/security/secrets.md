...
document, an issue, or the record.**
...
None of it is a credential and all of it is a map.

⛔ **Public is a property of every commit, not of the remote.** A tree that goes
public later publishes everything it ever held, so this section applies from the
first commit rather than from the first release.

⚠ **A pasted command output is the usual leak.** It is quoted because it is
evidence, and evidence carries the machine it was taken on. Read it before
pasting, and replace what names the host with something generic.

⛔ **A capture is subject to all of this.** It is data this project generated,
and generated data is still published data. That is why a capture is taken in a
machine that has visited nothing: [`../containers.md`](../containers.md).
