...
### ⛔ A pasted output is produced by running the command

⭐ **The worst thing this project can write is a plausible one**, and it has
written two.

A block of terminal output in a document, an entry or a commit message is a
**measurement**. It is produced by running the command and copying what came
back, in full, including the parts that are inconvenient. ⛔ Never composed from
what the command would print, never edited to remove a line, and never carried
forward from a previous run under a new date.

**What it cost, twice, on one tree.**

- An entry closed on a timing block reporting `gate ok: all 15 checks passed`.
  Measured the next day, on the tree that block names, two checks were failing.
  The output could not have come from the run it was labelled with, and its
  figures were copied into two scripts as the justification for a flag.
- Another closed on an acceptance command that cannot pass: a `grep` for a
  filename, written inside a file with that filename in it, which therefore
  always matches itself. The block underneath showed it finding nothing.

⚠ **Both were green, both read as evidence, and no check can see either.** The
work model rests on an entry closing with its acceptance command actually run
and its real output pasted underneath. A block nobody produced turns the record
from evidence into prose, and the next session cannot tell the difference.

⭐ **So: run it, paste it whole, and where a command genuinely cannot be run
here, write that instead.** "I could not run this, and here is what is needed"
is a complete and honest closing.
[`../methodology/gate.md`](../methodology/gate.md) says the same thing about a
...
because they feel like description. Replace the adjective with the measurement,
or delete it. "Fast" becomes the number and its conditions.

⛔ **Fourteen are always an assertion, and a check holds them.** A match is a
defect, every time, so it does not need a reading.
...
⚠ **Four more are banned as DISMISSALS and are a reading rather than a check.**

```text
simply    just    obviously    of course
```

They tell a reader who is stuck that what they cannot do is easy, and that is
the usage the rule refuses. ⭐ **They are also ordinary English in a contrast**,
as in "not just the names" or "none is obviously right", and no check can tell
the two apart. Measured over this tree on 2026-08-31, before the check existed:
nineteen matches on those four, every one legitimate, and no defects. ⚠ That is
one tree on one day and it moves with the prose. What does not move is the
ratio, which was nineteen to nothing. ⛔ A guard with
that false-positive rate is a guard somebody switches off, so
[`check-docs.sh`](../../scripts/common/check-docs.sh) holds the fourteen and
the review holds the four.
