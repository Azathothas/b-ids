...
A resume file, ⛔ **overwritten every session, never appended to.** It is a
dead man's switch, not a history. ⚠ **Where it lives is the project's choice
and the project's rules say where**, because a path written here would be a
second answer to a question the project has already answered.
...
5. **Close the entry with its evidence**, in place, per
   [`work-todo.md`](work-todo.md).
6. **Print the summary table.** Below.
7. **Print the next prompt, where the project asks for one.** Below. ⚠ Whether
   it does is the project's decision, and this repository's answer is no.
...
"Ran out of budget" is a reason. ⚠ In a project that asks for a prompt,
omitting it is not an option; in one that does not, the resume file carries the
same information and is on disk rather than in a chat log.
...
issue, re-run the full gate, update the entry.
...
Then read the record, read the entry that was in flight end to end, and check
the running system's actual state. Reconcile the three against each other:
