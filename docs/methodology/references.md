...
⚠ **The write-up belongs wherever the project has said it does, and it says so
once.** [`history.md`](history.md) is the general shape and names the departure
this repository made: a sweep read to do work sits beside the pages that answer
questions, not under the history directory. ⭐ The instrument goes in neither: it
is live tooling and it belongs with the project's other scripts, where the gate
can reach it.
