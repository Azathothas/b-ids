...
- **Self-review questions** answered honestly when the entry closes.
...
- **A small insert between two filed entries** takes the next integer too.
  ⚠ Ids are never reused and never renumbered: an id is how an entry has always
  been referred to, and a citation of it has to keep meaning what it meant.
- **A tiny fix** uses the same template, trimmed. ⛔ It still gets the full
  three-part gate and a closing. Do not skip the driven pass or the reviews
