...
The companion procedure for a unit of work inside a project that already
exists is [`authoring.md`](authoring.md).
...
| session management: starting sessions with the right context | writing the record and the next prompt |
...
what came before. [`work-todo.md`](work-todo.md) is the shape this project
uses; [`authoring.md`](authoring.md) is how each one is written.
...
read the ONE unit's plan -> restate it and its decisions in a few bullets
  ...
  -> close the entry with its evidence      docs/methodology/sessions.md
  -> print the summary table in chat, and whatever else the project asks for
...
- **A small deviation or clarification**, which does not move the unit's
  ...
  the entry with the reason.** A deviation from a locked decision is the
