...
### ⭐ 2026-09-04: the second ecosystem exists, and its acceptance runs

⚠ **The Prove named a script this tree did not have**, the third entry today
whose acceptance had to be written before it could be run.
...
```text
packages ok: 1 ecosystem(s) generated, 1 driven by their own runtime,
  each embedding 14 profile(s) and reporting release 6d2a751c71a9...,
  which is what sha256sum makes of the corpus index here.
  ⛔ Nothing generated fetches at runtime.
```

⚠ **The digest is ABBREVIATED in that paste and nowhere else.** The run printed
all sixty-four characters; a hex run that long in a tracked file is what
`check-no-secrets` refuses, and its exclusions are narrowed by name or by
path-and-shape rather than by whether a reader recognises a digest. ⛔ Widening
that rule so an entry could paste one would be the fix that removes the check.

⚠ **Exit 0, read from the process, unpiped.**

#### ⭐ Generated by the assembler, never hand-maintained

⛔ **A package somebody edits is a second copy of the corpus that drifts from
the first**, and the drift is invisible because both sides look like data. The
JavaScript package is written by `b_ids_corpus::packages` from the same corpus
every other published surface is built from, and it lands under
`packages/js/` in the published tree beside the formats and the routes.

| what it publishes | |
| --- | --- |
| `package.json` | ⛔ whose VERSION is `1.0.0-` and the identifier's first twelve, never a number somebody increments. Two builds over one corpus produce one version, and a hand-set one can disagree with the bytes it names |
| `corpus.json` | every profile, its published path, and its DERIVED platform token |
| `index.mjs` | `release`, `profiles`, `paths`, `at`, `select`, `latestStable`, `clientHelloHex` |

⭐ **The shape is `LIB-01`'s**, deliberately, so a consumer moving between the
two is not learning a second interface. That is `LIB-03`'s Must-not read from
the other side.

⛔ **The platform token is derived ONCE, in Rust, at generation time.** A profile
stores an operating system and an architecture; the token a route is built from
is a function of the two, and deriving it again in JavaScript would be a second
implementation of a rule this project already owns.

#### ⛔ The pin is recomputed rather than believed

⭐ **A third implementation, and that is the point.** The identifier is the
SHA-256 of the corpus index. `crates/b-ids`'s build script computes it with this
tree's own code, the package reports it, and the check recomputes it with
`sha256sum`, and on the PowerShell half with the platform's own hash. All three
agree, on a digest opening `6d2a751c71a9`.

⚠ **The index carries a digest of every published file**, so an identifier over
the index pins the corpus transitively: two builds reporting the same identifier
embedded the same bytes.

#### ⭐ Both guards were seen to refuse

⛔ **Planted against a copy, restored byte for byte, and the exit code read from
the process with no pipe.**

| planted | what it reported |
| --- | --- |
| the generator writes a wrong identifier | exit 1: "packages/js reports release 0000... and the corpus index digests to 6d2a..." |
| the generated source gains a `fetch(` | exit 1: "packages/js names fetch(, and a package that fetches at runtime is what PUB-05 forbids" |

⚠ **The second is this entry's Must-not**, checked over the generated source
rather than promised in a comment: a package that needs the network to answer is
a package that fails in the environment its consumers care most about.

#### ⚠ What the package was driven with, rather than reasoned about

Imported by `node` with no install step and no network, in the directory the
assembler wrote it to:

| asked | answered |
| --- | --- |
| `release().identifier` | the corpus index digest |
| `profiles().length`, `paths().length` | 14 and 14 |
| `latestStable('chrome', 'linux64')` | `chrome-152.0.7977.82-linux64-stable` |
| `latestStable('chrome', 'win64')` | `chrome-152.0.7977.83-win64-stable` |
| `select({browser:'chromium', platform:'linux64'})` | the one Chromium profile |
| ⛔ `latestStable('chrome', 'macos-arm64')` | **undefined.** A missing route is a fact and a substituted value is a lie |
| `clientHelloHex(...)` | 1951 bytes |

#### ⚠ What this does NOT do

⛔ **Nothing is published to a registry**, and nothing here can be: publishing
needs a credential, and the record's standing property is that nothing in this
tree needs one. ⭐ The package is BUILT and it is not PUSHED, which is the same
shape `HARNESS-12` closed under today.

⚠ **One ecosystem beside Rust, not "each major" one.** `ECOSYSTEMS` names what
exists rather than what is intended, and a row with no generator behind it would
be a claim rather than a package. ⛔ The Approach's "then add targets by demand
rather than by ambition" is the rule that keeps that honest.
