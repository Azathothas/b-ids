#!/bin/sh
# trust-anchor.sh - does trusting a real root change what the browser puts on
...
#
# Usage:
#   sh scripts/capture/trust-anchor.sh
#   sh scripts/capture/trust-anchor.sh --json
#   sh scripts/capture/trust-anchor.sh --headless --rounds 2
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) printf 'trust-anchor: unknown argument: %s\n' "$1" >&2; exit 2 ;;
  esac
  shift
done
case "$ROUNDS" in
  ''|*[!0-9]*) printf 'trust-anchor: --rounds needs a number\n' >&2; exit 2 ;;
esac
[ "$ROUNDS" -ge 1 ] || { printf 'trust-anchor: --rounds is at least 1\n' >&2; exit 2; }
...
command -v cargo >/dev/null 2>&1 || { printf 'trust-anchor: cargo not found\n' >&2; exit 2; }
command -v timeout >/dev/null 2>&1 || { printf 'trust-anchor: timeout not found\n' >&2; exit 2; }
...
if [ "$DISPOSABLE" != "1" ]; then
  printf 'trust-anchor: this machine is not marked disposable, so no root is installed.\n' >&2
...
OUT="$ROOT/.tmp/trust-anchor"
mkdir -p "$OUT" || exit 2

printf 'building\n'
cargo build -q -p b-ids-harness -p b-ids-driver || {
  printf 'trust-anchor: the workspace did not build\n' >&2
  exit 2
}
...
for exe in "$HARNESS" "$DRIVER"; do
  [ -x "$exe" ] || { printf 'trust-anchor: %s is not executable\n' "$exe" >&2; exit 2; }
done
...
"$DRIVER" resolve $BROWSER_FLAG --json > "$OUT/resolved.jsonl" 2>"$OUT/resolve.err" || {
  printf 'trust-anchor: nothing resolved. %s\n' "$(cat "$OUT/resolve.err")" >&2
  exit 2
}
  ...
  case "$PLATFORM" in
    ...
    MINGW*|MSYS*|CYGWIN*|Windows*)
      ...
      if [ -z "$WINDOWS_STORE" ]; then
        ...
        printf 'trust-anchor: no Windows store took the root. certutil said:\n' >&2
        cat "$OUT/store.log" >&2
        return 2
      fi
...
run_route() {
  ...
  if [ -z "$rr_base" ] || [ -z "$rr_pin" ]; then
    printf 'trust-anchor: %s printed no base URL or pin in 10s\n' "$rr_route" >&2
  ...
  if [ "$rr_route" = "pin" ]; then
    ...
    if ! install_root "$rr_ca"; then
      printf 'trust-anchor: could not install the root on this platform\n' >&2
      kill "$rr_pid" 2>/dev/null
      wait "$rr_pid" 2>/dev/null
      return 2
    fi
  ...
  wait "$rr_pid"
  if [ "$rr_route" != "pin" ]; then
    remove_root || printf 'trust-anchor: the root is STILL in the store after removal\n' >&2
  fi
...
if [ "${LEFT:-0}" != "0" ]; then
  printf 'trust-anchor: %s root(s) remain in the store\n' "$LEFT" >&2
  exit 1
fi
exit "$COMPARE_RC"
