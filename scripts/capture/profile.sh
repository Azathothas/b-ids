#!/bin/sh
# profile.sh - what does the browser on this machine put on the wire,
...
# docs/history/experiments/30-resumption-control.sh and it stopped being a condition every
...
#
# Usage:
#   sh scripts/capture/profile.sh
#   sh scripts/capture/profile.sh --headless
#   sh scripts/capture/profile.sh --browser edge
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) printf 'capture-profile: unknown argument: %s\n' "$1" >&2; exit 2 ;;
  esac
  shift
done
...
command -v cargo >/dev/null 2>&1 || { printf 'capture-profile: cargo not found\n' >&2; exit 2; }
command -v timeout >/dev/null 2>&1 || { printf 'capture-profile: timeout not found\n' >&2; exit 2; }
...
for exe in "$HARNESS" "$DRIVER" "$CORPUS"; do
  [ -x "$exe" ] || { printf 'capture-profile: %s is not executable\n' "$exe" >&2; exit 2; }
done
...
"$DRIVER" resolve $BROWSER_FLAG --json > "$OUT/resolved.jsonl" 2>"$OUT/resolve.err" || {
  printf 'capture-profile: nothing resolved. %s\n' "$(cat "$OUT/resolve.err")" >&2
  exit 2
}
...
if [ -z "$PIN" ] || [ -z "$BASE" ]; then
  printf 'capture-profile: the harness printed no pin or no base URL in 10s\n' >&2
  cat "$HARNESS_ERR" >&2
...
case "$TRUST_ROUTE" in
  ...
  *)
    printf 'capture-profile: the driver named no trust route: %s\n' \
...
if [ "$CONNECTIONS" = "0" ]; then
  printf 'capture-profile: the browser opened no connection this run\n' >&2
  ...
  printf 'capture-profile: what the browser wrote:\n' >&2
...
if [ -z "$RESUMPTION" ]; then
  printf 'capture-profile: the harness reported no resumption line\n' >&2
  exit 1
fi
...
' "$OUT/resolved.jsonl" "$OUT/switches.txt" "$IDENTITY" "${HEADLESS:-}" "$RESUMPTION" \
  ...
  printf 'capture-profile: could not write the identity file\n' >&2
  exit 1
}
