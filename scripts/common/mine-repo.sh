...
# above checks the OUTPUT DIRECTORY before anything is fetched, so it cannot
...
#
# ⭐ So the count is reported here, with the command that fixes it. It is a
# REPORT rather than a refusal: the tree is already fetched, and losing it to
# an exit code would be the corpus-deleting failure this script exists to stop.
IGNORED_COUNT=0
if git -C "$DEST" rev-parse --show-toplevel >/dev/null 2>&1; then
  IGNORED_COUNT=$(git ls-files --others --ignored --exclude-standard -- "$DEST" 2>/dev/null | grep -c . || true)
fi
if [ "${IGNORED_COUNT:-0}" -gt 0 ]; then
  printf '
⛔ %s file(s) under %s are on disk and would NOT be committed.
'     "$IGNORED_COUNT" "$DEST" >&2
  printf 'The mined tree carries its own ignore rules and git honours them, so
' >&2
  printf 'this part of the corpus exists on this machine and nowhere else.
' >&2
  printf 'Take it, and say so in the write-up:
