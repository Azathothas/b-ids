...
if ($UnboundArguments.Count -gt 0) {
    [Console]::Error.WriteLine('check-powershell: unknown argument: ' + $UnboundArguments[0])
    exit 2
}
...
if ($module) {
    try {
        ...
        # ⚠ NOTHING IS EXCLUDED FROM THE ANALYSER HERE, AND THE EXCLUSION
        # THAT USED TO BE HERE WAS DELETED RATHER THAN EMPTIED. It named three
        # directories holding the fragments of a generated script, none of which
        # exists in this tree. An exclusion for a path that does not exist grants
        # itself to whatever lands at that path next.
        #
        # ⭐ Where a generated script is ever added here, analyse the BUILT
        # product rather than its parts: a script-scoped suppression covers only
        # the file it is in, so a fragment reports every rule the product's own
        # suppressions exist to answer.
