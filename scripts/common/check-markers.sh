...
# .yml and .mjs in the tree unchecked. Measured where this check was written,
# 2026-08-28: 2290 characters outside the five, in 22 files, and every one of
# them was in a script rather than a document.
#
# ⭐ TWO FURTHER TREES REPRODUCED THAT SHAPE with their own numbers, both in the
# hundreds and both entirely in comment banners, while their markdown-only check
# reported them clean throughout. ⚠ This tree was not one of them and has no
# number of its own.
#
...
#   Azathothas/TEMPLATE                 9.0 overall, worst file 26.3
#   pkgforge-dev/cross-libc-dlopen      8.6 overall, worst file 21.8
#
# ⭐ The two trees either side of the middle row were ranked by eye before any
# of this was counted, and the ranking came out in that order. ⚠ Only those two
# were ranked; the tree the check was written in was not placed against them
# and its number simply falls between. One number reproducing a reading is the
# argument for having the number at all.
#
# ⚠ THIS TREE WAS NOT IN THAT SAMPLE and no density figure has been taken over
# it. What the sample does predict, from every tree that has run this check, is
# that the files nearest the ceiling are the RECORD files rather than the
# references, because a record earns its markers one incident at a time.
#
...
#
# -- THE EXEMPTION, FOR A REASON THAT WOULD OTHERWISE BREAK ------------------
#
