...
if ($UnboundArguments.Count -gt 0) {
    [Console]::Error.WriteLine('mine-repo: unknown argument: ' + $UnboundArguments[0])
    exit 2
}
...
# THE MINED TREE BRINGS ITS OWN .gitignore, AND git HONOURS IT. The guard
# earlier checks the OUTPUT DIRECTORY before anything is fetched, so it cannot
# see a rule that arrives inside the clone. Measured 2026-08-31: one mined tree
# carried '/bench/*.json', and 92 files of a corpus this repository states it
# keeps were on disk and in no commit. One of them was a primary evidence
# artefact cited twice in the write-up, and every local check passed because
# they resolve paths against the filesystem. CI is where it surfaced.
#
# So the count is reported here, with the command that fixes it. It is a REPORT
# rather than a refusal: the tree is already fetched, and losing it to an exit
# code would be the corpus-deleting failure this script exists to stop.
$ignoredCount = 0
try {
    $inRepo = (git -C $dest rev-parse --show-toplevel 2>$null)
    if ($LASTEXITCODE -eq 0 -and $inRepo) {
        $swallowed = @(git ls-files --others --ignored --exclude-standard -- $dest 2>$null | Where-Object { $_ })
        $ignoredCount = $swallowed.Count
    }
} catch { $ignoredCount = 0 }
if ($ignoredCount -gt 0) {
    [Console]::Error.Write("`n$ignoredCount file(s) under $dest are on disk and would NOT be committed.`n")
    [Console]::Error.Write("The mined tree carries its own ignore rules and git honours them, so`n")
    [Console]::Error.Write("this part of the corpus exists on this machine and nowhere else.`n")
    [Console]::Error.Write("Take it, and say so in the write-up:`n`n")
